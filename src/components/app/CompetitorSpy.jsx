    ...
    <div className="w-full space-y-3 font-sans">
      ...
      <div className="bg-[#171717] p-4 rounded-xl border border-[#262626] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-bold text-white font-sans">Competitor Keyword Gap Analysis</h1>
          <p className="text-xs text-zinc-400 mt-0.5">Audit rival domains to extract keywords they rank for that {domain} is missing.</p>
        </div>
      </div>

      {/* Input Form */}
      <div className="bg-[#171717] p-4 rounded-xl border border-[#262626] w-full">
        <form onSubmit={handleAuditCompetitor} className="flex gap-2">
          <input
            type="text"
      ...
      <div className="bg-[#171717] rounded-xl border border-[#262626] overflow-hidden w-full">
        {gaps.length === 0 ? (
          <div className="p-10 text-center bg-[#121212] space-y-3">
            ...
            <h3 className="text-sm font-bold text-white">Ready to Audit Competitor Gaps</h3>
            ...
            <table className="w-full text-left text-xs text-zinc-300">
              <thead className="bg-[#121212] text-zinc-400 uppercase tracking-wider font-semibold">
                <tr>
                  ...
                  <th className="p-3">Competitor Rank</th>
                  <th className="p-3">{domain} Rank</th>
              ...
              <tbody className="divide-y divide-[#262626]">
                {gaps.map((item) => (
                  <tr key={item.id} className="hover:bg-[#1F1F1F] transition-colors">
                    ...
                    <td className="p-3 font-semibold text-[#3ECF8E]">{item.competitorRank}</td>
                    <td className="p-3 text-zinc-400">{item.myRank}</td>
