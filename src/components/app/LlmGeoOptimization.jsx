...
export default function LlmGeoOptimization({ activeWebsiteUrl = 'mywebsite.com' }) {
  const [selectedBotDomain, setSelectedBotDomain] = useState(activeWebsiteUrl);
  const [isAuditingBots, setIsAuditingBots] = useState(false);
  const [botAuditResult, setBotAuditResult] = useState(null);

  const runBotAudit = (e) => {
    e.preventDefault();
    if (!selectedBotDomain) return;
    ...
    setTimeout(() => {
      setBotAuditResult({
        ...
        schemaScore: 'BlogPosting & Speakable Schema Present',
        entityConsensus: 'Listed on Verified Web Index'
      });
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-[#171717] p-6 rounded-2xl border border-[#262626] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-white font-sans">ChatGPT & Claude AI Search Visibility</h1>
          <p className="text-sm text-zinc-400 mt-1">Audit AI crawler access, entity authority, and information density across LLM answer engines.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-[#171717] p-6 rounded-2xl border border-[#262626] space-y-3">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Ensure AI crawlers like <strong className="text-white">GPTBot</strong>, <strong className="text-white">ClaudeBot</strong>, and <strong className="text-white">PerplexityBot</strong> are not blocked in your <code className="text-[#3ECF8E]">robots.txt</code> or WAF settings.
          </p>
        </div>
        ...
        <div className="bg-[#171717] p-6 rounded-2xl border border-[#262626] space-y-3">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Use <strong className="text-white">Bottom Line Up Front (BLUF)</strong> 40-word summary answers directly beneath question headers (H2/H3) for LLM text chunking.
          </p>
        </div>
        ...
        <div className="bg-[#171717] p-6 rounded-2xl border border-[#262626] space-y-3">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            LLMs rely on multi-source consensus. Build structured listings on <strong className="text-white">Wikidata</strong>, <strong className="text-white">Crunchbase</strong>, and <strong className="text-white">G2</strong>.
          </p>
        </div>
        ...
        <div className="bg-[#171717] p-6 rounded-2xl border border-[#262626] space-y-3">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            Real-time engines like Perplexity cite content with explicit <code className="text-[#3ECF8E]">dateModified</code> tags and current benchmarks.
          </p>
        </div>
        ...
        <div className="bg-[#171717] p-6 rounded-2xl border border-[#262626] space-y-3">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">
            LLM crawlers prefer clean semantic HTML tags (<code className="text-[#3ECF8E]">&lt;article&gt;</code>, <code className="text-[#3ECF8E]">&lt;table&gt;</code>) over heavy JS hydration.
          </p>
        </div>
      ...
      <div className="bg-[#171717] rounded-2xl border border-[#262626] p-6 space-y-4">
        <h3 className="text-base font-bold text-white font-sans">Live AI Bot Accessibility Diagnostic</h3>
        ...
        <form onSubmit={runBotAudit} className="flex flex-col sm:flex-row items-center gap-3">
          ...
          >
            {isAuditingBots ? 'Auditing AI Crawlers...' : 'Audit AI Bot Access'}
          </button>
        </form>

        {botAuditResult ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 pt-2 animate-in fade-in">
            ...
            <div className="p-3 bg-[#121212] rounded-xl border border-[#262626] text-sm">
              <span className="text-zinc-400 block text-sm">HTML Extractability</span>
              <span className="font-semibold text-[#3ECF8E]">{botAuditResult.rawHtmlExtractability}</span>
            </div>
          </div>
        ) : (
          <div className="p-6 bg-[#121212] rounded-xl border border-[#262626] text-center text-sm text-zinc-400">
            Enter your domain above and click <strong className="text-white">Audit AI Bot Access</strong> to test AI crawler accessibility.
          </div>
        )}
      </div>
